...
const InteractiveTextArea = ({isAuthenticated, comment, setComment, submitComment, placeholder, buttonText,disabled}: {isAuthenticated: boolean | undefined, comment: string, setComment: (arg0: string)=>void, submitComment: ()=>void, placeholder: string, buttonText:string, disabled: boolean}) => {
    ...
    <div dangerouslySetInnerHTML={{__html:"<b>sex</b>"}} placeholder={isAuthenticated ? placeholder : "You need to log in to comment."} contentEditable= {!disabled} className="overflow-y-scroll focus-visible:outline-none text-sm py-2 px-3 border-b-0 rounded-md-b-0 border focus-visible:border min-h-[80px] focus-visible:ring-0 focus-visible:ring-ring focus-visible:ring-offset-2">
        {/*comment*/}
    </div>
