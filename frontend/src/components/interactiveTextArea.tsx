...
const InteractiveTextArea = ({isAuthenticated, comment, setComment, submitComment, placeholder, buttonText,disabled, id, isLoading}: {isAuthenticated: boolean | undefined, comment: string, setComment: (arg0: string)=>void, submitComment: ()=>void, placeholder: string, buttonText:string, disabled: boolean, id: string, isLoading?: boolean}) => {
    ...
    <div role="textbox" className="flex border border-input bg-secondary p-2 items-center justify-end border-t-0 rounded-md-t-0">
        ...
        <Button disabled={!isAuthenticated || comment.length === 0 || comment.length > 500 || disabled} className="w-24 h-8 ml-2" onClick={submitComment} variant={"round"}>{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" /> } {buttonText}</Button>
