              ...
              <FormItem>
                ...
                <FormControl>
                  <div className="flex flex-row justify-end items-center">
                    <Input disabled={isLoading} {...field} value={community_name} onChange={(i)=>{setChecking(true);form.setValue("name",i.target.value); setCommunity_name(i.target.value);regex.test(i.target.value) ? checkAvailabilityDebounced(i.target.value).then(()=>{setChecking(false);setError(false);form.clearErrors("name")}).catch(()=>{setError(true);setChecking(false);form.setError("name", { message: "community already exists"})} ) : setChecking(false); form.setError("name", {message:"name must only contain letters and numbers and underscore"});setError(true)}}/>
